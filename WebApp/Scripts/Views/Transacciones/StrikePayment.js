...
function stripeTokenHandler(token) {
    ...
    $.ajax({
        cache: false,
        type: "POST",
        url: urlToApi + "/payment/EmpresaMakePayment",
        data: {
            CardToken: token.id,
            AmoutCrc: pago.Monto, //direct for testing this should be taken from the core and not UI information.
            Description: pago.Descripcion,
            LineaId: pago.LineaId,
            UserMail: auth.GetCurrentUser().Email,
            IdPago: pago.IdPago
        },
        success: function (result) {
            console.log(result);
            alert(result.Message);
            $("#acept").click(function () { window.location.replace("/Transaction/ListaDePagos") });
        },
...
function LoadElements() {
    ...
    var pago = JSON.parse(sessionStorage.getItem("listaTransacciones_selected"));
    $("#Linea").text(pago.Linea.Empresa.NombreEmpresa + ": " + pago.Linea.NombreLinea);
    $("#txtDescripcion").text(pago.Descripcion);
    $("#txtEspacios").text(pago.Linea.EspaciosParqueo);
    $("#txtMonto").text(pago.Monto);
    Retrieve();
}
