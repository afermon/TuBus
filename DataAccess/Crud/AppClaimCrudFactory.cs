...
{
    public class AppClaimCrudFactory : CrudFactory
    {
        AppClaimMapper _mapper;

        public AppClaimCrudFactory() : base()
        {
            _mapper = new AppClaimMapper();
        ...
        public List<T> RetrieveByView<T>(string viewId)
        {
            var lstItems = new List<T>();

            var lstResult = dao.ExecuteQueryProcedure(_mapper.GetRetriveByVistaStatement(viewId));
